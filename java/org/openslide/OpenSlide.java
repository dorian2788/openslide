...
public final class OpenSlide implements Closeable {
    ...
    public void paintRegionOfLevel(Graphics2D g, int dx, int dy, int sx,
            int sy, int plane, int w, int h, int level) throws IOException {
        paintRegion(g, dx, dy, plane, sx, sy, w, h, levelDownsamples[level]);
    }

    // takes the reader lock
    public void paintRegionARGB(int dest[], long x, long y, long plane, int level, int w,
        ...
        try {
            checkDisposed();
            OpenSlideJNI.openslide_read_region(osr, dest, x, y, plane, level, w, h);
            checkError();
        } finally {
            rl.unlock();
        }
    ...
    public void paintRegion(Graphics2D g, int dx, int dy, long sx, long sy, long plane,
        ...
        paintRegionARGB(data, baseX, baseY, plane, level, img.getWidth(), img
                .getHeight());
    ...
    public BufferedImage createThumbnailImage(int x, int y, long plane, long w, long h,
        ...
        paintRegion(g, 0, 0, sx, sy, plane, sw, sh, ds);
        g.dispose();
        return result;
    }
    ...
    public BufferedImage createThumbnailImage(int x, int y, long plane, long w, long h,
        ...
        return createThumbnailImage(x, y, plane, w, h, maxSize,
                BufferedImage.TYPE_INT_RGB);
    }

    public BufferedImage createThumbnailImage(int maxSize, long plane) throws IOException {
        return createThumbnailImage(0, 0, plane, getLevel0Width(), getLevel0Height(),
                maxSize);
    }
